...
export async function POST(request: NextRequest) {
  try {
    ...
    // Log the full error to Vercel Function Logs so we can diagnose it
    // (Vercel Dashboard → Deployments → Functions → click the route).
    console.error("[api/quiz/answers] grading failed:", err);

    const message =
      err instanceof Error ? err.message : "Unknown error grading answer";
