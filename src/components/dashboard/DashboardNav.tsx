...
export default function DashboardNav({ totalXp, streak }: DashboardNavProps) {
  ...
  return (
    <header className="bg-slate-950/90 backdrop-blur-xl sticky top-0 z-50">
      {/* `relative` on the bar so the absolutely-centered nav anchors here. */}
      <div className="container mx-auto px-4 sm:px-6 h-14 sm:h-16 flex items-center justify-between max-w-6xl relative">
        {/* Left: Brand only — nav was pulled out and centered separately. */}
        ...
        {/* Center: nav links — absolutely positioned so they sit at the true
            visual midpoint of the bar regardless of how wide the right-side
            HUD chip cluster grows. Hidden below `md:` (mobile menu takes over). */}
