# StudyQuest — Agent Changelog (detailed sprint history)

This file holds the dated, forensic sprint history that used to live in `CLAUDE.md`.
Split out on 2026-06-03 to keep `CLAUDE.md` lean (it loads into context every session).
CLAUDE.md keeps the durable rules, current-state summary, and migrations list; this file
keeps the per-feature / per-commit detail. Read it for the backstory behind a feature.
