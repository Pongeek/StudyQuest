...
- 👁️ **Truesight Calibration Coach** - a profile section that measures how well your self-rated confidence matches your actual correctness. Per-tier accuracy bars (guessed / unsure / confident), signal chips for *overconfident stumbles* (confident but wrong) and *lucky guesses* (guessed but right), a plain-language verdict (well-calibrated / overconfident / underconfident / mixed), and a blind-spot drill-down that links straight to the topics you're most overconfident about. Stays in "cold-start" until you've rated enough answers, so the read is never noisy.
...
- 🏅 **Featured Trophy** - pin any earned achievement to your profile hero crest; falls back to your most-recently-earned trophy when you haven't picked one
